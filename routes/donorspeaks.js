...
donorspeaksRouter.route('/:donorspeaksId')
.options((req,res)=>{res.sendStatus(200);})
.get((req,res,next) => {
...
.put(authenticate.verifyUser,authenticate.roleAuthorization(['admin','donor']), (req, res, next) => {
    ...
    .then((donorspeaks) => {
        if (donorspeaks != null) {
            ...
            req.body.author = req.user._id;
            DonorSpeaks.findByIdAndUpdate(req.params.donorspeaksId, {
                $set: req.body
            }, { new: true })
            .then((donorspeaks) => {
                DonorSpeaks.findById(donorspeaks._id)
...
.delete(authenticate.verifyUser,authenticate.roleAuthorization(['donor','admin']),(req, res, next) => {
    ...
    .then((donorspeaks) => {
        if (donorspeaks != null) {
            ...
            err = new Error('Blog with id ' + req.params.donorspeaksId + ' not found');
            err.status = 404;
            return next(err);            
        }
    }, (err) => next(err))
